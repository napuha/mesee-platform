...
class _ProfilePageState extends State<ProfilePage> {
  late Future<List<PostRecord>> posts; Map<String, dynamic>? profile; String selected = 'own';
  @override void initState() { super.initState(); _reload(); }
  void _reload() { posts = selected == 'own' ? const PostRepository().fetchOwnPosts() : selected == 'vertical_video' ? const PostRepository().fetchOwnPosts(kind: 'vertical_video') : selected == 'horizontal_video' ? const PostRepository().fetchOwnPosts(kind: 'horizontal_video') : selected == 'text' ? const PostRepository().fetchOwnPosts(kind: 'text') : const PostRepository().fetchReactionPosts(selected); const PostRepository().fetchCurrentProfile().then((value) { if (mounted) setState(() => profile = value); }); }
  ...
  @override Widget build(BuildContext context) => ListView(padding: const EdgeInsets.all(16), children: [
    Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
      ...
      const CircleAvatar(radius: 42, child: Logo()),
    ]),
    const SizedBox(height: 20), const Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [Text('12\nフォロー中', textAlign: TextAlign.center), Text('732\nフォロワー', textAlign: TextAlign.center), Text('2.7万\nいいね', textAlign: TextAlign.center)]), const SizedBox(height: 20),
...
class _ProfileEditDialogState extends State<_ProfileEditDialog> {
  final name = TextEditingController(), username = TextEditingController(), bio = TextEditingController(); bool busy = false, isPrivate = false; String? error;
  @override void initState() { super.initState(); _load(); }
  ...
  Future<void> save() async { setState(() { busy = true; error = null; }); try { await const PostRepository().updateProfile(displayName: name.text, username: username.text, bio: bio.text, isPrivate: isPrivate); if (mounted) Navigator.pop(context); } catch (exception) { if (mounted) setState(() => error = exception.toString()); } finally { if (mounted) setState(() => busy = false); } }
  @override void dispose() { name.dispose(); username.dispose(); bio.dispose(); super.dispose(); }
  @override Widget build(BuildContext context) => AlertDialog(title: const Text('プロフィールを編集'), content: Column(mainAxisSize: MainAxisSize.min, children: [TextField(controller: name, decoration: const InputDecoration(labelText: '名前')), TextField(controller: username, decoration: const InputDecoration(labelText: '@ユーザーネーム')), TextField(controller: bio, maxLines: 3, decoration: const InputDecoration(labelText: '自己紹介')), SwitchListTile(contentPadding: EdgeInsets.zero, title: const Text('非公開アカウント'), value: isPrivate, onChanged: busy ? null : (value) => setState(() => isPrivate = value)), if (error != null) Text(error!, style: const TextStyle(color: Colors.redAccent))]), actions: [TextButton(onPressed: busy ? null : () => Navigator.pop(context), child: const Text('キャンセル')), FilledButton(onPressed: busy ? null : save, child: Text(busy ? '保存中…' : '保存'))]);
}
